...
-- Copy and paste this directly into your Supabase project SQL Editor
-- =====================================================================
...
CREATE POLICY "categories_admin_all" ON categories FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());

-- PRODUCTS: Public can read active, admin can read & write all
CREATE POLICY "products_select" ON products FOR SELECT USING (active = true OR public.is_admin());
CREATE POLICY "products_admin_all" ON products FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());

-- SERVICES: Public can read active, admin can read & write all
CREATE POLICY "services_select" ON services FOR SELECT USING (active = true OR public.is_admin());
CREATE POLICY "services_admin_all" ON services FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());

-- ORDERS: Public CANNOT read orders. Only admin can read.
CREATE POLICY "orders_select_admin" ON orders FOR SELECT TO authenticated USING (public.is_admin());
-- Public storefront can only INSERT pending orders
CREATE POLICY "orders_insert_public" ON orders FOR INSERT WITH CHECK (status = 'pending' AND amount_paid = 0);
-- Only admin can update orders (the Paystack webhook uses service_role key to update orders safely server-side)
CREATE POLICY "orders_update_admin" ON orders FOR UPDATE TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
CREATE POLICY "orders_delete_admin" ON orders FOR DELETE TO authenticated USING (public.is_admin());

-- ORDER ITEMS: Only admin can read, public can insert during checkout
CREATE POLICY "order_items_select_admin" ON order_items FOR SELECT TO authenticated USING (public.is_admin());
CREATE POLICY "order_items_insert_public" ON order_items FOR INSERT WITH CHECK (true);
CREATE POLICY "order_items_admin_modify" ON order_items FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());

-- ADMIN USERS: Only admins can view/manage
CREATE POLICY "admin_users_select" ON admin_users FOR SELECT TO authenticated USING (id = auth.uid() OR public.is_admin());
CREATE POLICY "admin_users_modify" ON admin_users FOR ALL TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());
...
ON CONFLICT (slug) DO UPDATE SET starting_price = EXCLUDED.starting_price, deposit_amount = EXCLUDED.deposit_amount;
