...
(function () {
  ...
  function setEntry(segment) {
    try {
      localStorage.setItem("tovila_entry", segment);
    } catch (e) {}
  }
  function getEntry() {
    try {
      return localStorage.getItem("tovila_entry");
    } catch (e) {
      return null;
    }
  }
  ...
  if (burger && menu) {
    ...
    menu.querySelectorAll(".m-close").forEach(function (b) {
  ...
  document.querySelectorAll("[data-multistep]").forEach(function (form) {
    ...
    if (segment && entry === segment && total > 1) {
      idx = 1;
      if (doneWrap) {
        doneWrap.textContent =
          "\u2713 " + (SEGMENT_LABELS[entry] || "Your selection") + " \u2014 step 1 complete";
  ...
  if (quiz) {
    var TIPS = {
      sediment: {
        ...
        tips: [
          "Your supply is most likely carrying suspended solids from a borehole screen or an ageing tank — a sediment (multimedia) filter upstream of everything else usually clears it.",
      ...
      taste: {
        ...
        tips: [
          'A chlorine or "swimming pool" taste points to municipal residual — activated carbon removes it cheaply and immediately.',
      ...
      hard: {
        ...
        tips: [
          "White spots on taps and glassware mean dissolved calcium and magnesium. Every 1mm of scale inside a heater raises its energy use noticeably and shortens its life.",
      ...
      none: {
        ...
        tips: [
          "Clear, odourless water can still be microbiologically unsafe or high in dissolved solids — neither is visible.",
    ...
    quiz.addEventListener("submit", function (e) {
      ...
      out.innerHTML =
        '<span class="eyebrow">Free instant diagnostic</span><h3>' +
        data.title +
        "</h3><ul>" +
        data.tips
          .map(function (t) {
  ...
  if (builder) {
    ...
    function renderPackage() {
      ...
      if (!on.length) {
        ...
        slots.innerHTML = on
          .map(function (b) {
            ...
            return (
              '<div class="slot"><span class="s-ico">' +
              c.ico +
              "</span><span>" +
              c.name +
              "</span></div>"
      ...
      if (count)
        count.textContent = on.length + " component" + (on.length === 1 ? "" : "s") + " selected";
    }
  ...
  if (pathBuilder) {
    ...
    stages.forEach(function (s, i) {
      s.addEventListener("click", function () {
        ...
        if (out) {
          out.innerHTML =
            '<span class="eyebrow">Your path</span><h3>' +
            (stages.length - i) +
            " stages, starting at " +
            ...
            stages
              .slice(i)
              .map(function (x) {
                return x.getAttribute("data-stage");
              })
              .join(" \u2192 ") +
  ...
  if (tabs.length && subject) {
    ...
    function selectTab(value) {
      ...
      tabs.forEach(function (t) {
        t.classList.toggle("on", t.getAttribute("data-subject") === value);
      });
    ...
    tabs.forEach(function (t) {
      t.addEventListener("click", function () {
        selectTab(t.getAttribute("data-subject"));
      });
    });
    subject.addEventListener("change", function () {
      selectTab(subject.value);
    });
    selectTab(
      Array.prototype.some.call(tabs, function (t) {
        return t.getAttribute("data-subject") === want;
      })
        ? want
        : "General Enquiry",
    );
  ...
  document.querySelectorAll(".play").forEach(function (b) {
    b.addEventListener("click", function () {
      ...
      if (note)
        note.textContent =
          "Placeholder — Lesson 1 video embed pending final upload. No signup required when live.";
    });
  });
})();
