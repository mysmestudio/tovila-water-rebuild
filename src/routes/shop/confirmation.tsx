      ...
      <main className="flex-1 max-w-4xl mx-auto px-4 sm:px-6 py-10 w-full">
        {loading ? (
          <div className="py-24 text-center">
            ...
            <p className="text-xs text-slate-500 mt-3 font-medium">Verifying order status...</p>
          </div>
        ) : error || !order ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-8 text-center max-w-lg mx-auto">
            ...
            <Link
              to="/shop"
              className="inline-block px-4 py-2 bg-[#0a2540] text-white text-xs font-semibold rounded-lg"
            >
              Return to Catalog
            </Link>
          </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-sm">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-100">
                <div className="flex items-center gap-3.5">
                  {order.status === "paid" ? (
                    <div className="w-12 h-12 rounded-full bg-emerald-50 text-emerald-600 border border-emerald-200 flex items-center justify-center shrink-0">
                      <CheckCircle2 className="w-7 h-7" />
                    </div>
                  ) : (
                    <div className="w-12 h-12 rounded-full bg-amber-50 text-amber-600 border border-amber-200 flex items-center justify-center shrink-0">
              ...
              {isSimulated && order.status === "pending" && (
                <div className="mt-6 p-4 bg-amber-50 border border-amber-300 rounded-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  ...
                  <button
                    ...
                    disabled={confirming}
                    className="w-full sm:w-auto shrink-0 bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold px-4 py-2 rounded-lg transition shadow-sm"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-6 pt-2">
                <div>
                  ...
                  <p className="text-xs text-slate-800 font-bold">{order.customer_name}</p>
              ...
              <div className="mt-8">
                ...
                <div className="border border-slate-200 rounded-xl overflow-hidden">
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {order.items?.map((it) => (
                        <tr key={it.id}>
                          <td className="p-3 font-semibold text-slate-900">{it.name_snapshot}</td>
                          <td className="p-3 text-center">
                            <span className="capitalize text-[10px] bg-slate-100 px-2 py-0.5 rounded font-medium">
                              {it.item_type}
                            </span>
                          </td>
                ...
                <div className="mt-4 p-4 bg-slate-50 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
                  <div>
              ...
              {/* Next Steps Card */}
              <div className="mt-8 p-5 bg-cyan-50/60 border border-cyan-200 rounded-xl text-xs text-slate-700">
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 flex flex-wrap items-center justify-between gap-4">
                <button
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2 border border-slate-200 rounded-lg text-xs font-semibold text-slate-700 hover:bg-slate-50 transition"
                >
                ...
                <div className="flex items-center gap-3">
                  <Link
                    to="/shop"
                    className="inline-flex items-center gap-1.5 bg-[#0a2540] hover:bg-[#184b7a] text-white px-4 py-2 rounded-lg text-xs font-bold transition shadow-sm"
                  >
